import grpc, random, threading, api_pb2_grpc, api_pb2, argparse

def get_target_port(key):
    with grpc.insecure_channel('localhost:30051') as channel:
        stub = api_pb2_grpc.ShardManagerStub(channel)
        response = stub.GetShardIndex(api_pb2.KeyRequest(key=key))
        return response.port


def run(operation, key, value):
    try:
        group_port = get_target_port(key)
        with grpc.insecure_channel(f"localhost:{group_port}") as channel:
            stub = api_pb2_grpc.RaftKeyValueStub(channel)
