...
def serve(server_num, grp_port, raft_port):
    partner_addresses = []
    for n in range(3):
        if n != server_num:
            partner_addresses.append(f"localhost:{raft_port+n}")
    repl_dict = ReplDict()
    SyncObj(f"localhost:{raft_port+int(server_num)}", partner_addresses, consumers=[repl_dict])
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=1))
    api_pb2_grpc.add_RaftKeyValueServicer_to_server(RaftKeyValue(repl_dict), server)
    
    ## assume that master is server num = 0
    ## only 1 rpc for each group is exposed
    if server_num == 0:
        server.add_insecure_port(f"[::]:{grp_port+int(server_num)}")
    try:
        server.start()
        print(f"Server started, listening on {grp_port+int(server_num)}")
